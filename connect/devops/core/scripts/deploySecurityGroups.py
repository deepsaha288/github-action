...
if __name__ == '__main__':
    ...
    parser.add_argument("--JenkinsNodeCIDR", dest="JenkinsNodeCIDR", required=True, metavar="<JenkinsNodeCIDR>",
                        help="Jenkins node CIDR")
    # parser.add_argument("--s3Prefix", dest="s3Prefix", required=True, metavar="<s3Prefix>",
    #                     help="S3 Prefix List ID")
    ...
    try:       
        ...
        input_params['JenkinsNodeCIDR'] = args.JenkinsNodeCIDR
        current_region = boto3.client('ec2').meta.region_name
        input_params['s3Prefix'] = boto3.client('ec2').describe_prefix_lists(
                    Filters=[{'Name': 'prefix-list-name','Values': ['com.amazonaws.'+current_region+'.s3']}]
                    )['PrefixLists'][0]['PrefixListId']
        # input_params['s3Prefix'] = args.s3Prefix
        ...
        cloudformation.create_stack(StackName=input_params['stackName'],\
                                    ...
                                    Parameters=[{'ParameterKey':'VpcId','ParameterValue':input_params['vpcId']},
                                                {'ParameterKey':'JenkinsNodeCIDR','ParameterValue':input_params['JenkinsNodeCIDR']}]
                                    )
